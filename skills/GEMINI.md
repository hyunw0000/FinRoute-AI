# GEMINI.md — FinRoute-AI 시스템 파이프라인 인터페이스 완전 정의서

<project_overview>
이 문서는 01~05 파일 간의 데이터 흐름과 반환 형태를 정의하는 전체 파이프라인 인터페이스 정의서이다. Gemini는 구현 시 이 인터페이스 구조를 100% 엄격히 준수해야 하며, 어떠한 기술적 세부 사항도 누락하거나 요약해서는 안 된다.
</project_overview>
...
| 프레임워크 | Streamlit |
| 차트 | Plotly |
| LLM | Gemini 1.5 Pro (preferred) / Claude Sonnet / GPT-4o |
| 분류 | scikit-learn (cosine_similarity) |
| 진입점 | app.py |
| API 키 | .env 파일의 ANTHROPIC_API_KEY 또는 GEMINI_API_KEY 환경변수로 관리 |
</technical_stack>
...
<module name="indicator_calculator.py">
...
<module name="insight_generator.py">
...
<module name="dashboard_builder.py">
...
<global_rules>
## 프로젝트 전역 준수 규칙
1. 모든 파일 수정 시 `skills/` 폴더 내의 상세 규칙(.md)을 최우선으로 준수한다.
2. 데이터 인터페이스는 위 정의된 구조를 100% 엄격히 따른다 (딕셔너리 키 이름/구조 변경 금지).
3. **Action Console 레이아웃**: 반드시 **세로(상-하) 구조**로 렌더링하며, 대시보드 화면(4-8 섹션)에는 리밸런싱/헤지 제안을 표시하지 않는다.
4. 모든 모듈은 `00_interface.md`의 데이터 흐름을 충실히 따른다.
</global_rules>
